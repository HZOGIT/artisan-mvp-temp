...
describe("devis-nouveau — domain pur", () => {
  ...
  it("totals : HT/TVA/TTC multi-lignes", () => {
    const t = totals([ligne({ quantite: 2, prixUnitaireHT: 100, tauxTVA: 20 }), ligne({ quantite: 1, prixUnitaireHT: 50, tauxTVA: 10 })]);
  ...
  it("ligneFromArticle : TVA article sinon courante", () => {
    ...
    expect(ligneFromArticle(ligne({ tauxTVA: 20 }), art)).toMatchObject({ description: "Robinet", prixUnitaireHT: 75, tauxTVA: 5.5 });
    expect(ligneFromArticle(ligne({ tauxTVA: 20 }), { ...art, tauxTVA: null }).tauxTVA).toBe(20);
  });
  it("iaToLignes + iaTotals", () => {
    ...
    expect(iaToLignes(prop)[0]).toMatchObject({ description: "D", quantite: 2, prixUnitaireHT: 100, tauxTVA: 20 });
    expect(iaTotals(prop.lignes)).toEqual({ ht: 200, ttc: 240 });
  });
  it("buildAddLignePayload / buildModeleLignePayload", () => {
    expect(buildAddLignePayload(7, ligne({ description: "L", quantite: 3, prixUnitaireHT: 10, tauxTVA: 20 }))).toEqual({ devisId: 7, designation: "L", quantite: "3", prixUnitaireHT: "10", tauxTVA: "20" });
    expect(buildModeleLignePayload(5, ligne({ description: "L", quantite: 3, prixUnitaireHT: 10, tauxTVA: 20, unite: "u" }))).toEqual({ modeleId: 5, designation: "L", quantite: 3, prixUnitaireHT: 10, tauxTVA: 20, unite: "u" });
  });
});
