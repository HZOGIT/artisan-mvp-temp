...
const STATUT_COLORS: Record<string, string> = {
  ...
  annulee: "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300",
};
...
export function UpcomingInterventionsWidget() {
  ...
  return (
    <div className="space-y-3">
      {list.map((intervention: any, i: number) => {
        ...
        return (
          <motion.div
            ...
            className="flex items-start gap-3 rounded-lg border border-border/60 bg-background/60 p-3 hover:bg-accent/40 cursor-pointer transition-colors"
          >
            <div className="h-9 w-9 shrink-0 rounded-lg bg-orange-100 dark:bg-orange-900/30 inline-flex items-center justify-center">
              <Calendar className="h-4 w-4 text-orange-600 dark:text-orange-400" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-medium truncate">{intervention.titre || "Intervention"}</p>
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${statutCls}`}>
                  {statutLabel}
                </span>
              </div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-3 text-[11px] text-muted-foreground">
                <span className="inline-flex items-center gap-1">
                  <Clock className="h-3 w-3" /> {formatDateHeure(intervention.dateDebut)}
                </span>
