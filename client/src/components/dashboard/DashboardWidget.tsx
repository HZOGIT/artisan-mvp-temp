...
/**
 * Conteneur statique de widget pour le dashboard.
 *
 * Le drag & drop a été retiré (4 tentatives infructueuses sur le contexte
 * Dashboard Operioz, fix non prioritaire). Le widget reste masquable via
 * la croix en haut à droite. L'ordre et la visibilité sont persistés dans
 * localStorage côté Dashboard.tsx via le panneau "Personnaliser".
 */
export function DashboardWidget({
  ...
  return (
    <div
      data-widget-id={id}
      className={`relative bg-card text-card-foreground rounded-xl border border-border shadow-sm hover:shadow-md transition-shadow cursor-default ${
        className || ""
      }`}
    >
