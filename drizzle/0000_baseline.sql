...
CREATE TABLE `ai_threads` (
	...
	CONSTRAINT `ai_threads_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
CREATE TABLE `contrats_maintenance` (
	...
	`description` text,
	`type` enum('maintenance_preventive','entretien','depannage','contrat_service') DEFAULT 'entretien',
	...
	`preavisResiliation` int DEFAULT 1,
...
CREATE TABLE `conversations` (
	...
	`nonLuArtisan` int DEFAULT 0,
	`nonLuClient` int DEFAULT 0,
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()),
...
CREATE TABLE `factures` (
	...
	`statut` enum('brouillon','validee','envoyee','payee','en_retard','annulee') DEFAULT 'brouillon',
	`typeDocument` enum('facture','avoir') DEFAULT 'facture',
	`factureOrigineId` int,
	`objet` text,
	`conditionsPaiement` text,
	`notes` text,
	`totalHT` decimal(10,2) DEFAULT '0.00',
...
CREATE TABLE `interventions_contrat` (
	...
	`statut` enum('planifiee','en_cours','effectuee','annulee') DEFAULT 'planifiee',
	`rapport` text,
	`notes` text,
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `interventions_contrat_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
CREATE TABLE `messages` (
	...
	`auteur` enum('artisan','client') NOT NULL,
...
CREATE TABLE `permissions_utilisateur` (
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `permissions_utilisateur_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
CREATE TABLE `rdv_en_ligne` (
	...
	`dureeEstimee` int DEFAULT 60,
	`statut_rdv` enum('en_attente','confirme','refuse','annule') DEFAULT 'en_attente',
	`motifRefus` text,
	`urgence_rdv` enum('normale','urgente','tres_urgente') DEFAULT 'normale',
	`interventionId` int,
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `rdv_en_ligne_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
CREATE TABLE `suivi_chantier` (
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `suivi_chantier_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
CREATE TABLE `users` (
	...
	`role` enum('admin','artisan','secretaire','technicien') NOT NULL DEFAULT 'artisan',
	`artisanId` int,
