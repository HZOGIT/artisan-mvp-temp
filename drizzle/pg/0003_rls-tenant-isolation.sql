-- Custom migration — RLS isolation multi-tenant (rapatriée de drizzle/rls/tenant-isolation.sql).
-- Générée par scripts/rls/generate-tenant-rls.mjs puis figée ici comme migration versionnée.
-- Idempotente (DROP POLICY IF EXISTS + CREATE) → rejouable sans danger sur une base déjà dotée.
-- Appliquée par drizzle-kit migrate sous le rôle owner (artisan_user). NE PAS ÉDITER une fois appliquée :
-- une nouvelle table tenant = régénérer le SQL et créer une NOUVELLE migration custom (append).
...
create policy tenant_isolation on "audit_log" using ("artisanId" = nullif(current_setting('app.tenant', true), '')::int) with check ("artisanId" = nullif(current_setting('app.tenant', true), '')::int);
