...
async function findArtisanByUserId(connection, userId) {
  try {
    ...
    console.error('Error finding artisan:', error.message);
    return null;
  }
}
...
async function createClient(connection, artisanId, clientData) {
  try {
    const [result] = await connection.query(
      'INSERT INTO clients (artisanId, nom, prenom, email, telephone, adresse, codePostal, ville, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())',
      [artisanId, clientData.nom, null, clientData.email, clientData.telephone, clientData.adresse, clientData.codePostal, clientData.ville]
    );
...
async function createDevis(connection, artisanId, clientId, index) {
  try {
    ...
    const [result] = await connection.query(
      'INSERT INTO devis (artisanId, clientId, numero, dateDevis, totalHT, totalTVA, totalTTC, statut, createdAt, updatedAt) VALUES (?, ?, ?, NOW(), ?, ?, ?, ?, NOW(), NOW())',
...
async function createFacture(connection, artisanId, clientId, index) {
  try {
    ...
    const [result] = await connection.query(
      'INSERT INTO factures (artisanId, clientId, numero, dateFacture, dateEcheance, totalHT, totalTVA, totalTTC, statut, createdAt, updatedAt) VALUES (?, ?, ?, NOW(), DATE_ADD(NOW(), INTERVAL 30 DAY), ?, ?, ?, ?, NOW(), NOW())',
...
async function createIntervention(connection, artisanId, clientId, clientData, index) {
  try {
    const dateDebut = new Date(Date.now() + (index * 7 * 24 * 60 * 60 * 1000));
    
    const [result] = await connection.query(
      'INSERT INTO interventions (artisanId, clientId, titre, description, dateDebut, statut, adresse, notes, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())',
      [artisanId, clientId, `Intervention ${index} - ${clientData.nom}`, `Description de l'intervention ${index}`, dateDebut, 'planifiée', clientData.adresse, `${clientData.codePostal} ${clientData.ville}`]
    );
    return result.insertId;
  } catch (error) {
...
async function runTests() {
  ...
  try {
    ...
    if (!results.artisanId) {
      console.log(`⚠️  Profil artisan non trouvé. Création en cours...`);
      try {
        const [result] = await connection.query(
          'INSERT INTO artisans (userId, nomEntreprise, specialite, createdAt, updatedAt) VALUES (?, ?, ?, NOW(), NOW())',
          [results.userId, 'Artisan Test', 'multi-services']
        );
        results.artisanId = result.insertId;
        console.log(`✅ Profil artisan créé (ID: ${results.artisanId})`);
      } catch (error) {
        console.log(`❌ Erreur création profil artisan: ${error.message}`);
        process.exit(1);
      }
    } else {
