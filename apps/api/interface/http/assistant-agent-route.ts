  ...
  app.post("/api/assistant/stream", async (req, reply) => {
    ...
    try {
      ...
      if (e instanceof TooManyRequestsError) {
        req.log.warn({ event: "ia_rate_limit", artisanId: auth.artisanId }, "IA agent rate limit hit");
        return reply.code(429).send({ error: e.message });
      }
      req.log.error({ event: "assistant_agent_error", error: e instanceof Error ? e.message : String(e) }, "Assistant agent failed");
