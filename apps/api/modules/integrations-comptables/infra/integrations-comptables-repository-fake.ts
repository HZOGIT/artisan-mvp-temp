...
export class IntegrationsComptablesRepositoryFake implements IIntegrationsComptablesRepository {
  ...
  constructor(state: IntegrationFakeState = {}) {
    ...
    this.pendingItems = state.pendingItems ?? [];
  }
  ...
  async listSyncLogs(_ctx: TenantContext): Promise<ExportComptableRow[]> {
    return [...this.exports].sort((a, b) => b.id - a.id).slice(0, 50);
  }
  async listPendingItems(_ctx: TenantContext): Promise<PendingItem[]> {
