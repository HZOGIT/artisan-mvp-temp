import { describe, it, expect, afterAll, beforeAll } from "vitest";
import { SignJWT } from "jose";
import { Pool } from "pg";
import { buildApp } from "../../../../app";
import { createDbClient } from "../../../../shared/db";
import { DrizzleTenantResolver } from "../../../../shared/tenant/drizzle-tenant-resolver";
import { injectTrpc } from "../../../../shared/testing/trpc-inject";

/** L3 authz — gate notes_frais.approuver sur approuver/rejeter/payer (OPE-1015). */

const URL = process.env.DATABASE_URL;
const APP_URL =
  process.env.APP_DATABASE_URL ||
  (URL ? URL.replace(/:\/\/[^@]+@/, "://app_tenant:app_tenant_pw@") : undefined);
const SECRET = "test-secret-at-least-32-characters-long-xxxx";

/** owner du compte artisan */
const OWNER = 9899901;
/** collaborateur MEMBRE non-owner rattaché au même artisan (anti-674 : pas l'owner) */
const MEMBER = 9899902;

const jwt = (userId: number) =>
  new SignJWT({ userId, email: `u${userId}@t.fr` })
    .setProtectedHeader({ alg: "HS256" })
    .setExpirationTime("1h")
    .sign(new TextEncoder().encode(SECRET));

function mut(app: ReturnType<typeof buildApp>, path: string, input: unknown, tok?: string) {
  return injectTrpc(app, "POST", path, input, tok);
}

describe.skipIf(!URL)("notesDeFrais.router authz — gate notes_frais.approuver (OPE-1015)", () => {
  const admin = new Pool({ connectionString: URL });
  const appDb = createDbClient(APP_URL!);
  let server: ReturnType<typeof buildApp>;

  const cleanup = async () => {
    await admin.query('delete from permissions_utilisateur where "userId" in ($1,$2)', [OWNER, MEMBER]);
    await admin.query('delete from notes_de_frais where artisan_id in (select id from artisans where "userId"=$1)', [OWNER]);
    await admin.query('delete from artisans where "userId"=$1', [OWNER]);
    await admin.query("delete from users where id in ($1,$2)", [OWNER, MEMBER]);
  };

  beforeAll(async () => {
    await cleanup();
    await admin.query("insert into users (id, email, password, role) values ($1,$2,'x','artisan')", [OWNER, `u${OWNER}@t.fr`]);
    const { rows } = await admin.query<{ id: number }>('insert into artisans ("userId") values ($1) returning id', [OWNER]);
    const artisanId = rows[0].id;
    await admin.query(
      'insert into users (id, email, password, role, "artisanId") values ($1,$2,\'x\',\'artisan\',$3)',
      [MEMBER, `u${MEMBER}@t.fr`, artisanId],
    );
    server = buildApp({ jwtSecret: SECRET, resolver: new DrizzleTenantResolver(appDb.db) });
  });

  afterAll(async () => {
    await server.close();
    await cleanup();
    await appDb.close();
    await admin.end();
  });

  it("approuver — membre sans notes_frais.approuver → 403", async () => {
    const tok = await jwt(MEMBER);
    expect((await mut(server, "notesDeFrais.approuver", { id: 999999 }, tok)).statusCode).toBe(403);
  });

  it("approuver — owner bypasse la garde → non-403 (404 note absente)", async () => {
    const tok = await jwt(OWNER);
    expect((await mut(server, "notesDeFrais.approuver", { id: 999999 }, tok)).statusCode).not.toBe(403);
  });

  it("approuver — membre AVEC notes_frais.approuver → non-403", async () => {
    await admin.query('insert into permissions_utilisateur ("userId",permission,autorise) values ($1,$2,true)', [MEMBER, "notes_frais.approuver"]);
    const tok = await jwt(MEMBER);
    expect((await mut(server, "notesDeFrais.approuver", { id: 999999 }, tok)).statusCode).not.toBe(403);
    await admin.query('delete from permissions_utilisateur where "userId"=$1 and permission=$2', [MEMBER, "notes_frais.approuver"]);
  });

  it("rejeter — membre sans notes_frais.approuver → 403", async () => {
    const tok = await jwt(MEMBER);
    expect((await mut(server, "notesDeFrais.rejeter", { id: 999999, commentaire: "Justificatif manquant" }, tok)).statusCode).toBe(403);
  });

  it("rejeter — owner bypasse la garde → non-403 (404 note absente)", async () => {
    const tok = await jwt(OWNER);
    expect((await mut(server, "notesDeFrais.rejeter", { id: 999999, commentaire: "Justificatif manquant" }, tok)).statusCode).not.toBe(403);
  });

  it("rejeter — membre AVEC notes_frais.approuver → non-403", async () => {
    await admin.query('insert into permissions_utilisateur ("userId",permission,autorise) values ($1,$2,true)', [MEMBER, "notes_frais.approuver"]);
    const tok = await jwt(MEMBER);
    expect((await mut(server, "notesDeFrais.rejeter", { id: 999999, commentaire: "Justificatif manquant" }, tok)).statusCode).not.toBe(403);
    await admin.query('delete from permissions_utilisateur where "userId"=$1 and permission=$2', [MEMBER, "notes_frais.approuver"]);
  });

  it("payer — membre sans notes_frais.approuver → 403", async () => {
    const tok = await jwt(MEMBER);
    expect((await mut(server, "notesDeFrais.payer", { id: 999999 }, tok)).statusCode).toBe(403);
  });

  it("payer — owner bypasse la garde → non-403 (404 note absente)", async () => {
    const tok = await jwt(OWNER);
    expect((await mut(server, "notesDeFrais.payer", { id: 999999 }, tok)).statusCode).not.toBe(403);
  });

  it("payer — membre AVEC notes_frais.approuver → non-403", async () => {
    await admin.query('insert into permissions_utilisateur ("userId",permission,autorise) values ($1,$2,true)', [MEMBER, "notes_frais.approuver"]);
    const tok = await jwt(MEMBER);
    expect((await mut(server, "notesDeFrais.payer", { id: 999999 }, tok)).statusCode).not.toBe(403);
    await admin.query('delete from permissions_utilisateur where "userId"=$1 and permission=$2', [MEMBER, "notes_frais.approuver"]);
  });
});
