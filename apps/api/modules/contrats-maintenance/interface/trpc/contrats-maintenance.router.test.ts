...
describe.skipIf(!URL)("contrats.router e2e (HTTP → tRPC → use-case → repo → RLS + état machine)", () => {
  ...
  beforeAll(async () => {
    for (const uid of [UA, UB]) {
      ...
      for (const __p of ["contrats.voir", "contrats.gerer"]) await admin.query('insert into permissions_utilisateur ("userId", permission, autorise) values ($1,$2,true)', [uid, __p]);
    }
