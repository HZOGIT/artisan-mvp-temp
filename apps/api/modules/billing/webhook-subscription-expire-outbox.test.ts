...
describe.skipIf(!URL)("billing outbox atomicité L2 (abonnement.expire via handleSubscriptionDeleted)", () => {
  ...
  beforeAll(async () => {
    ...
    const subRow = await admin.query(
      `insert into billing_subscriptions (artisan_id,plan_id,billing_mode,status,trial_ends_at)
       values ($1,'starter','stripe','trialing',now() + interval '30 days') returning id`,
      [artisanId],
    );
  ...
  it("updateSubscriptionStatus canceled et abonnement.expire co-écrits dans la même transaction", async () => {
    const repo = new BillingRepositoryDrizzle(appDb.db);
    const ctx = { artisanId, userId: 0 };

    await withOutbox(appDb.db, repo, async (r) => {
      await r.updateSubscriptionStatus(ctx, "canceled");
    ...
    expect(subRow.rows[0].status).toBe("canceled");

    const evRow = await admin.query(
      `select * from event_outbox where action='abonnement.expire' and "artisanId"=$1`,
      [artisanId],
    );
