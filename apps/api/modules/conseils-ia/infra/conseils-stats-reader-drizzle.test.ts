...
describe.skipIf(!URL)("ConseilsStatsReaderDrizzle (agrégats best-effort, scopés tenant sous RLS)", () => {
  ...
  beforeAll(async () => {
    ...
    // A : 2 devis en attente (brouillon+envoye), 1 accepté (exclu) ; 2 factures impayées (envoyee+en_retard), 1 payée (exclue) ; 1 stock bas
  ...
  it("A : compte devis en attente (2), factures impayées (2, total 150), stocks bas (1)", async () => {
