...
async function startServer() {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // DIAGNOSTIC TEMPORAIRE — verification end-to-end de la config Stripe.
  // A SUPPRIMER apres validation manuelle. Ne renvoie JAMAIS les valeurs
  // secretes, uniquement leur presence + des metadonnees Stripe non-sensibles
  // (montant, interval) qui sont deja publics dans le checkout.
  // ──────────────────────────────────────────────────────────────────────────
  app.get('/api/stripe/config-check', async (_req, res) => {
    const result: any = {
      env: {},
      db: {},
      stripe: {},
    ...
    // 1) Presence des env vars (sans valeur).
    const envVars = [
      'APP_URL',
      'STRIPE_SECRET_KEY',
    ...
    for (const v of envVars) {
      result.env[v] = process.env[v] ? '✅ Set' : '❌ Missing';
    }

    // 2) Existence des tables DB + count.
    try {
      const db = await import('../db');
      const pool = db.getPool();
      if (pool) {
        for (const table of ['subscriptions', 'devices', 'active_sessions']) {
          try {
            const [rows] = await pool.execute(`SELECT COUNT(*) AS cnt FROM ${table}`) as any;
            result.db[table] = { status: '✅ OK', count: Number((rows as any[])[0].cnt) };
          } catch (e: any) {
            result.db[table] = { status: '❌ MISSING', error: e?.message };
          }
        }
        // Subscription artisan id=1.
        try {
          const [rows] = await pool.execute(
            `SELECT plan, status, max_users, max_devices_per_user,
                    max_concurrent_sessions, trial_ends_at, stripe_customer_id IS NOT NULL AS has_customer
             FROM subscriptions WHERE artisan_id = 1`
          ) as any;
          result.db.artisan1 = (rows as any[])[0] || null;
        } catch (e: any) {
          result.db.artisan1 = { error: e?.message };
        }
        // Count artisans.
        try {
          const [rows] = await pool.execute('SELECT COUNT(*) AS cnt FROM artisans') as any;
          result.db.artisans_total = Number((rows as any[])[0].cnt);
        } catch (e: any) {
          result.db.artisans_total = { error: e?.message };
        }
      } else {
        result.db.error = 'Pool indisponible';
      }
    } catch (e: any) {
      result.db.error = e?.message;
    }

    // 3) Verification des price IDs Stripe (server-side appel API).
    if (process.env.STRIPE_SECRET_KEY) {
      try {
        ...
        for (const envName of priceEnvs) {
          const priceId = process.env[envName];
          if (!priceId) {
            result.stripe[envName] = '❌ Missing env';
            continue;
          }
          try {
            const price = await stripeClient.prices.retrieve(priceId);
            result.stripe[envName] = {
              status: '✅ OK',
              id: price.id,
            ...
            result.stripe[envName] = { status: '❌ Stripe error', error: e?.message };
          }
        }
      } catch (e: any) {
      ...
      result.stripe.error = 'STRIPE_SECRET_KEY non defini';
    }
