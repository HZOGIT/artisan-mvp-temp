...
export class FakeAssistantThreadWriter implements AssistantThreadWriter {
  ...
  public messages: Array<{ artisanId: number; threadId: number; role: string; transcript: string }> = [];
  ...
  async addMessage(ctx: TenantContext, threadId: number, role: "user" | "assistant", transcript: string): Promise<void> {
    this.messages.push({ artisanId: ctx.artisanId, threadId, role, transcript });
  }
}
