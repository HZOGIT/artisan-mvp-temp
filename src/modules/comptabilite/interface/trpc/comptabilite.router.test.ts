...
describe.skipIf(!URL)("comptabilite.router e2e (gardé par permission)", () => {
  ...
  beforeAll(async () => {
    ...
    await admin.query('insert into artisans ("userId","nomEntreprise") values ($1,$2)', [UID, "Compta SARL"]);
  ...
  it("authentifié SANS la permission `comptabilite.voir` → 403", async () => {
    expect((await injectTrpc(app, "GET", "comptabilite.getBalance", undefined, await jwt(UID))).statusCode).toBe(403);
  });
