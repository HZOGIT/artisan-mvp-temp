...
describe("edge dispatch (functions/_lib/dispatch.mjs) — parité avec le gateway src", () => {
  ...
  it("domaine non porté → legacy même si listé (sûreté)", () => {
    // `supportZZZ` n'est pas (encore) porté dans le new-stack → reste legacy même listé via env.
    expect(decideTarget("/api/trpc/supportZZZ.list", { NEW_STACK_DOMAINS: "supportZZZ" })).toBe("legacy");
  });
