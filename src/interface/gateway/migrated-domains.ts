...
export const STAGING_NEW_STACK_DEFAULT_DOMAINS = [
  ...
  "vitrine", // parité vérifiée : getBySlug/submitContact (PUBLIC par slug) + getDemandesContact/updateDemandeContactStatut/convertirDemandeEnClient (ADMIN leads). getBySlug : artisan résolu par slug (artisans HORS RLS) → page agrégée (params vitrine + avis publiés + stats + catégories) lue sous le scope tenant résolu (404 si inconnu/inactif). submitContact : anti-flood IP (5/15min) + email artisan + notif + persistance lead (best-effort). Admin : délégation `demandesContact` (+ `clients` pour la conversion), ownership scopé tenant.
  "clientPortal", // parité vérifiée : 18 procs. ADMIN (cookie) : generateAccess/getStatus/deactivate. PUBLIC (TOKEN, RLS public-token) : . resolveByToken (actif+non expiré) → lectures scopées tenant + filtrées clientId (anti-IDOR) ; chat = compose IChatRepository migré (double anti-IDOR) ; soumettreDemandeIA = LlmPort + dégradation propre ; anti-flood IP/token sur les mutations.
  "integrationsComptables", // parité vérifiée : 10 procs (tous protected). getConfig/saveConfig/saveSyncConfig/getSyncStatus/getExports/genererExport + getSyncLogs/getPendingItems/lancerSync/retrySync. Tables config/exports SOUS RLS (upsert whitelisté). genererExport FEC réutilise le générateur compta migré (Σdébit=Σcrédit, LECTURE SEULE) ou IIF QuickBooks pur. Sync = création d'exports + suivi des factures non couvertes (aucune écriture mutée). getPendingItems renvoie l'objet attendu par le client (corrige le tableau nu legacy).
  "devisIA", // parité vérifiée : 7 procs (tous protected). list/getById/createAnalyse/addPhoto/updateSuggestion + analyserPhotos (Vision multi-image) + genererDevis. analyses_photos_chantier SOUS RLS, tables filles scopées via l'analyse parente. analyserPhotos : VisionPort.analyzeImages (data:URL→inline / http→fileData) + prompt métier + persist résultats/suggestions (match biblio) + statut termine/erreur. genererDevis : devis brouillon (lignes/totaux dérivés) depuis suggestions sélectionnées. ⚠️ updateSuggestion COMBLE un IDOR latent du legacy (anti-IDOR suggestion→résultat→analyse).
  // C4 (dispatcher mono-stack) : les 8 sous-ressources new-stack (jamais appelées top-level par le
  // client — cf. inventaire C2) sont activées pour que `DEFAULT_ENABLED == MIGRATED_DOMAINS` → AUCUN
  // domaine porté ne tombe en legacy. Activation sans effet sur le trafic client actuel.
