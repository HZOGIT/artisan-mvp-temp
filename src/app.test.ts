...
describe.skipIf(!HAS_DB)("app Fastify (scaffold + tRPC)", () => {
  ...
  it("un domaine non migré (integrationsComptables) n'est PAS monté → 404", async () => {
    const res = await app.inject({ method: "GET", url: "/api/trpc/integrationsComptables.list" });
    expect(res.statusCode).toBe(404);
  });
});
