...
describe.skipIf(!HAS_DB)("app Fastify (scaffold + tRPC)", () => {
  ...
  it("un domaine non migré (vitrine) n'est PAS monté → 404", async () => {
    const res = await app.inject({ method: "GET", url: "/api/trpc/vitrine.list" });
    expect(res.statusCode).toBe(404);
  });
});
