...
**Prochaine cible : DURCISSEMENT (a) suite** — scanner d'autres use-cases critiques sans test L1 dédié. Pistes : `convertirDevisEnFacture` (gardes devis non-accepté / déjà converti), `creerAvoir` branches restantes, ou côté devis/paiement. Méthode : `grep "export async function"` du use-case vs son test, couvrir les branches manquantes. 1 fichier/itération.
