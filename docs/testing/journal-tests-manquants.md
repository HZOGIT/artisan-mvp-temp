...
**Prochaine cible : DURCISSEMENT (a) suite** — repérer d'autres endpoints/branches critiques non couverts en L3 (procédures publiques ou à garde forte non testées via le vrai app). Méthode : pour un routeur critique, lister ses `.query/.mutation` vs les cas du `*.router.test.ts`, couvrir la procédure manquante. 1 fichier/itération.
