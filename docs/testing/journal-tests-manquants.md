...
**Prochaine cible : DURCISSEMENT (a) suite** — branches catch best-effort restantes. Piste : `envoyer-relance-facture` (escalade niveau, déjà payée → skip, échec email best-effort) ou `envoyer-facture-email` (branche d'échec EmailPort non bloquante). Méthode : lire le use-case + son test, exercer la branche d'erreur/skip non couverte. 1 fichier/itération. ⚠️ Valeur marginale décroissante (saturation structurelle depuis it.78) — répété à l'humain it.79 ; si rien de significatif au prochain réveil → basculer L4 (b) ou clôturer.
