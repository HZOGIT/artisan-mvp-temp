...
**Prochaine cible : DURCISSEMENT (a) suite** — autres branches résiduelles des chemins critiques. Pistes restantes : arrondi/résilience du calcul **devis** (`devis/application/montants` ou équivalent, mêmes gardes que factures), branches catch best-effort (relance email, notif), erreurs d'adaptateurs. Méthode : lire le module + son test, exercer la branche non couverte. 1 fichier/itération. ⚠️ Valeur marginale décroissante (couverture structurelle saturée depuis it.78) → si la prochaine cible n'apporte pas de garantie réelle, basculer L4 (b) ou signaler clôture à l'humain.
