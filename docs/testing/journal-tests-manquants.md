...
**Prochaine cible : DURCISSEMENT (a) suite** — chercher d'autres **branches résiduelles** des chemins critiques (catch/erreur, garde rare, calcul limite) non couvertes. Pistes : branches catch best-effort (emails/notifs), erreurs d'adaptateurs (stripe/llm), bornes de calcul (TVA multi-taux, arrondis). Méthode : lire le use-case critique + son test, repérer la branche non exercée. 1 fichier/itération. Si plus rien de significatif → envisager L4 (b) ou signaler saturation à l'humain.
