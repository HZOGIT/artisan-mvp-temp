...
**Prochaine action : P0.7d-8** (subscriptions — billing/Stripe : `getSubscription` ~L4613, `updateSubscription` ~L4659, `getSubscriptionByCustomerId` ~L4699. ⚠️ tables subscriptions hors schema.ts legacy → vérifier modélisation pg + intégrité plan/limites). Puis 7d-9 devices, 7d-10 sessions, 7d-11 getDashboardStats. Puis routers.ts/index.ts. Puis P0.8 (copie data) + P0.9 (tests PG) avant cutover.
