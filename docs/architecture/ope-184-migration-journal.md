...
**Prochaine action : P0.7d-10** (sessions : `getActiveSessions`/`countActiveSessions`/`createSession`/`deleteOldestSession`/`cleanExpiredSessions` ~L4866-4936, table `active_sessions` pg). Puis 7d-11 getDashboardStats (L2042, dernière fn ensurePool). Puis routers.ts/index.ts. Puis P0.8 (copie data) + P0.9 (tests PG) avant cutover.
