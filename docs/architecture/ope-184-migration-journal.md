...
**→ 🎯 db.ts raw-SQL COMPLET : plus AUCUN `getPool()`/`ensurePool()` dans server/db.ts** (getDashboardStats L2042 utilisait `ensurePool()` — à VÉRIFIER : c'est la seule restante). 

**Prochaine action : P0.7d-11** (`getDashboardStats` L2042 — vérifier si encore en raw SQL via ensurePool, porter en Drizzle si oui ; sinon db.ts est 100% Drizzle). Puis audit `routers.ts`/`index.ts` (getPool d'après audit initial : 5+8). Puis P0.8 (copie data) + P0.9 (tests PG) avant cutover.
