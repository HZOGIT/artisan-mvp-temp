...
## Prochaine action : **R0.17 — Gateway (aiguillage ancien/nouveau stack par flag)**
Implémenter le **gateway** qui route les préfixes tRPC `<domaine>.*` vers l'ancien ou le nouveau stack selon un **feature flag** (granularité par domaine, et idéalement par tenant) : proxy Cloudflare `functions/api/[[path]].js` (ou un middleware Express devant le legacy) qui, selon le flag, transmet la requête au nouveau service Fastify (`/api/trpc`) ou au legacy. Flag **off par défaut** (le legacy continue de tout servir). Créer `src/interface/gateway/` (logique de décision pure + testable : `shouldRouteToNewStack(domaine, tenantId, flags)`), et le câblage proxy. Gate `tsc src` + test unitaire de la décision (flag off → legacy ; flag on pour domaine X → nouveau). Pas de `OPE-XXX` en code. Puis 4 canaux. Ensuite : R0.19 feature flags (table/env lus par le gateway), R0.14-15 CI, R0.16 migrations hors boot, QW deps mortes — puis **phases 1→5** (gabarit vehicules OPE-208→216, 1ère vraie migration de domaine).
