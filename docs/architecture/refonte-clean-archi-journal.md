...
## Prochaine action : **R0.13 — Ports d'effets de bord (OPE ~0.13)**
Définir dans `src/shared/ports/` les interfaces des effets de bord pour découpler les use-cases de l'infra : `EmailPort` (sendEmail({to,subject,body})), `SmsPort` (sendSms({to,message})), `StoragePort` (put/get/delete/url), `PdfPort` (render(template,data)→Buffer/stream). + au moins un **adapter** par port réutilisant l'existant (ex. `EmailPort` → wrap `server/_core/emailService`), et un **fake/in-memory** par port pour les tests. Gate `tsc src` + test (un use-case fictif appelant un EmailPort fake → vérifie l'appel). Pas de `OPE-XXX` en code. Puis 4 canaux. Ensuite Phase 0 : R0.18 scaffold Fastify (src/app.ts + /health), R0.14-15 CI, R0.16 migrations hors boot, R0.17 gateway, R0.19 feature flags, QW deps mortes — puis phases 1→5 (gabarit vehicules OPE-208→216).
