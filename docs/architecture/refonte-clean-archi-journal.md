...
## Prochaine action : **R0.18 — Scaffold Fastify (src/app.ts + /health)**
Créer le squelette du nouveau stack serveur **Fastify** (décision verrouillée) : `src/app.ts` qui construit une instance Fastify, expose **`GET /health`** (200 `{status:'ok'}`), et un `src/server.ts` (ou `buildApp()` + `listen`) bootable. Ajouter la dép `fastify` si absente (`package.json`). Pas encore d'adapter tRPC ni de routes métier (viendront avec le gateway R0.17 + les domaines). Gate `tsc src` + test (`app.inject({method:'GET',url:'/health'})` → 200) — Fastify `inject` permet de tester sans port. Pas de `OPE-XXX` en code. Puis 4 canaux. Ensuite Phase 0 restante : R0.14-15 CI, R0.16 migrations hors boot, R0.17 gateway, R0.19 feature flags, QW deps mortes — puis phases 1→5 (gabarit vehicules OPE-208→216).
