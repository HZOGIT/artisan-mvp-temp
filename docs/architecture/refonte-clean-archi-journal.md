...
## Prochaine action : **R0.18b — Adapter tRPC sur Fastify + @fastify/cookie + appRouter vide**
Compléter le scaffold (DoD OPE-204) : ajouter `@fastify/cookie` + `@trpc/server` adapter Fastify, monter un **`appRouter` tRPC vide** (ou avec un `health`/`ping` procedure) sur **`/api/trpc`**. Créer `src/interface/trpc/` (router racine + contexte tRPC qui construira le `TenantContext` via le kernel R0.10 — extraction cookie `token` → claims → resolver ; resolver DB = stub pour l'instant). Ajouter les déps manquantes (host + conteneur). Gate `tsc src` + test : `app.inject` sur une procedure tRPC vide → 200. Pas de `OPE-XXX` en code. Puis 4 canaux + marquer **OPE-204 Done**. Ensuite Phase 0 restante : R0.17 gateway (flag proxy Cloudflare), R0.19 feature flags, R0.14-15 CI, R0.16 migrations hors boot, QW deps mortes — puis phases 1→5 (gabarit vehicules OPE-208→216).
