...
## Prochaine action : **P1 vehicules 2/9 — Repository Drizzle scopé tenant (OPE-209)**
Implémenter `src/modules/vehicules/infra/vehicule-repository-drizzle.ts` : `VehiculeRepositoryDrizzle implements IVehiculeRepository`, **toutes les requêtes via `dbForTenant(ctx)`/`withTenant`** (R0.11) + **filtre `artisanId` explicite** (RLS en filet). Mapper les lignes Drizzle (table `vehicules` du schema.pg) ↔ types de domaine. Pour entretiens/assurances : vérifier d'abord que le `vehiculeId` appartient au tenant (sinon null). `updateKilometrage` : invariant **compteur ne recule pas** (GREATEST/condition). Importer les tables depuis `drizzle/schema.pg` (le nouveau stack est PG-only). Gate `tsc src` + **test d'intégration PG** : repo du tenant A ne lit aucune ligne du tenant B (via le rôle app non-superuser + RLS, comme `rls-real-table.test.ts`), CRUD basique. Pas de `OPE-XXX` en code. Puis 4 canaux → 3/9 use-cases lecture (OPE-210, harnais cross-tenant R0.20), etc.
