...
formatRelativeDate (5 tests, 0 any). **Brique 2 ✅** : `buildSidebarGroups` (compose perms+modules+drop vides) + `isPathActive`/`resolveActiveGroup`/`resolveActiveItem` (resolveV2Path injecté = pur) + `MOBILE_PRIMARY` + `rail-colors.ts` (RAIL_COLORS 8 couleurs dont purple). 4 tests. La couche DOMAIN du shell est complète + testée (vitest.v2 464). **Brique 3 ✅** : `modern/shell/ui/sidebar.tsx` — sidebar PRÉSENTATIONNELLE pilotée par props (rail desktop + panneau étendu), port fidèle, 0 dépendance legacy/tRPC, i18n namespace `shell`, badges injectés (railBadge/itemBadge). NB: le suite vitest.v2 est pure-logique (pas de -library) → pas de render-test ; validation visuelle au câblage. **Brique 4 ✅** : `modern/shell/ui/mobile-nav.tsx` — nav MOBILE (barre du bas MOBILE_PRIMARY + drawer « Plus » à groupes repliables), port fidèle, props-driven (0 dépendance legacy), i18n shell. **Brique 5 ✅** : `modern/shell/ui/dashboard-layout.tsx` — CONTAINER qui assemble Sidebar + MobileNav + top bar (titre actif + bouton menu mobile) + slots (topBarActions/banners/assistant). Gère l'état nav (openGroupId, mobileMore, openMobileGroups) + handlers (railGroupClick 1-item→nav, navigate via resolveV2Path, toggleMobileGroup). DÉCOUPLÉ : données (permissions/modules/user/resolveV2Path/onNavigate) + zones tRPC (recherche/notifs/bannières/assistant) INJECTÉES par props/slots. **Brique 6 ✅** : `use-shell.ts` (couche application : user+permissions via auth.me, modulesActifs via modules.getMine, logout) + `userInitial` pur testé. tsc.v2 confirme auth.me expose name/email/permissions. **Brique 7 ✅** : `dashboard-layout-mount.tsx` — branche useShell + navigation wouter (useLocation/setLocation, même méca que legacy+pages /v2) + resolveV2Path, enveloppe `<Outlet/>` TanStack dans DashboardLayout. Slots (recherche/notifs, bannières, assistant FAB+drawer) = props à fournir au câblage.

⚠️ GATE PARITÉ AVANT CUTOVER : câbler le shell modern avec slots VIDES = perte de NotificationBell + GlobalSearch + AssistantFAB/Drawer + TrialBanner + ExpiredBlocker (régression). Avant le cutover, il faut FOURNIR ces slots — soit en portant ces features tRPC en modern/, soit en les bridgeant. Le câblage final (router root → DashboardLayoutMount + App.tsx ne wrappe plus /v2 en legacy) ne se fait QU'APRÈS parité des slots, + deploy + validation navigateur complète.
