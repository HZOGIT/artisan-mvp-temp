...
## 🎯 PROCHAINE CIBLE : longue traîne 30/30 migrée. **À LA REPRISE, dans l'ordre :**
1. **Réintégrer la section « réglages vitrine » dans `/v2/parametres`** dès que les endpoints `vitrine.getSettings`/`updateSettings` sont livrés (OPE-504, **décision humaine du 2026-06-17 : on crée ces endpoints pour parachever**). Détail dans l'entrée `parametres` ci-dessous (« À FAIRE À LA REPRISE »). Vérifier si OPE-504 est passé en Done / si les procédures existent dans `src/modules/vitrine/interface/trpc/` avant de coder.
2. Puis les GROS chantiers différés (Dashboard 16 widgets + dé-batch / Portail client Stripe / Home / DashboardAdvanced) — voir liste ci-dessous.
3. Dette : portage propre d'`AbonnementSection` (Stripe/devices) vers `@/modern/shared/trpc`.
