...
## 🎯 PROCHAINE CIBLE : **cutover + bascule v2 par défaut + suppression legacy** (cf. « ETA GLOBAL »). Toutes les pages sont migrées. Phases : (1) cutover lien portail backend → `/v2/portail` ; (2) bascule v2 par défaut (retrait opt-in `?v2=1`) + sweep global ; (3) suppression `client/src/pages/**` + routing wouter + `@/lib/trpc` une fois inutilisés.
