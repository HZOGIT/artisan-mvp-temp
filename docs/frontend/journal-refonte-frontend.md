...
## 🎯 PROCHAINE CIBLE : **Vague R — feature `commandes`** (rétrofit clean-archi). Extraire
`domain/commande.ts` (types `RouterOutputs` + règles pures : filtres statut/fournisseur, libellés/totaux)
+ `application/use-commandes.ts` (SEULE couche tRPC : list + delete/sendEmail + invalidation), faire
consommer `commandes-page.tsx` (plus de `trpc` direct, **0 `any`**, warning `no-trpc-in-ui` disparaît).
**Audit §3bis 6/6 obligatoire + consigné.** Garder parité e2e verte. Gabarit = `use-articles` /
`use-fournisseurs`. Reste après : stocks, depenses, comptabilite, signature. Quand les 5 warnings
