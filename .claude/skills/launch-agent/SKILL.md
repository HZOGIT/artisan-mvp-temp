---
name: launch-agent
description: Lancer une session agent Claude en arrière-plan (screen détaché + remote-control + permissions auto). À utiliser pour déléguer une tâche à une session worker ou monter une session infra (reviewer, project-manager). Détaille modèles, nommage, prompts d'init.
---
